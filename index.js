...
function extractContent(html) {
  ...
  const images = [...new Set($('img').map((i, element) => $(element).attr('src')).get())];
  const urls = [...new Set($('a').map((i, element) => $(element).attr('href')).get())];
...
function saveContent(content, baseUrl) {
  ...
  content.images.forEach((imgSrc, index) => {
    ...
    const imgPath = path.join(imagesDir, `image_${index}.jpg`);
  ...
  const sortedUrls = content.urls.sort();
  fs.writeFileSync(path.join(urlsDir, 'urls.txt'), sortedUrls.join('\n'));
}
